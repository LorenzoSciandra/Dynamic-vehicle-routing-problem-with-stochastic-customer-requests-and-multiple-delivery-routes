...
double CostFunctionSddp::GetCost(Sol<Node, Driver> &s) {
    ...
    for (int i = 0; i < s.GetUnassignedCount(); i++) {
        ...
        if (n->type == NODE_TYPE_PICKUP) {
            k++;
            if (n->is_real) k += 100;
            if (n->action_type == DECISION_ACTION_GO_NOW || n->action_type == DECISION_ACTION_WAIT ||
                n->action_type == DECISION_ACTION_DONT_UNASSIGN)
                k += 100;
        }
    }
...
double CostFunctionSddp::GetCost(Sol<Node, Driver> &s, Driver *d) {
    ...
    while (prev->type != NODE_TYPE_END_DEPOT) {
        ...
        if (Parameters::ForbidStochasticDropAfterReal() && next->type == NODE_TYPE_DROP && !next->is_real &&
            prev->is_real)
            return BIG_DOUBLE;
        ...
        if (next->type == NODE_TYPE_PICKUP && next->action_type == DECISION_ACTION_WAIT &&
        ...
        if (next->type == NODE_TYPE_PICKUP && next->action_type == DECISION_ACTION_GO_NOW &&
            time > Parameters::GetCurrentTime() + 0.01) {
        ...
        if (go_now_count >= 1 &&
            (next->release_time > Parameters::GetCurrentTime() || next->action_type == DECISION_ACTION_WAIT)) {
    ...
    while (cur != NULL) {
        ...
        if (cur->IsCustomer()) {
            ...
            if (cur->type == NODE_TYPE_PICKUP)
                ...
                printf("d");
            printf("(ac:%d rel:%.0lf arr:%.1lf tw:%.0lf-%.0lf)-", cur->action_type, cur->release_time,
                   cur->arrival_time, cur->tw_start, cur->tw_end);
