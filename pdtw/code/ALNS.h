        ...
        for (int iter = 0; iter < _iterator_count; iter++) {
            ...
            if (show_output && (iter % 1000) == 0)// || best_cost > newcost)
                printf("Iter:%d rmv:%d newcost:%.2lf(%d,%d) cost:%.2lf best:%.2lf T:%.8lf\n",
                       iter, nbremove, newcost, cur.GetUnassignedCount(), (int) cur.IsFeasible(), curr_cost, best_cost,
                       T);
            ...
            if (best_cost > newcost && cur.IsFeasible()) {
                ...
                if (curr_cost > newcost) {
                    nb_improved++;
                    avg_gap_improved += gap;
                }
                else {
                    nb_worst++;
                    if (cur.GetUnassignedCount() == 0) avg_gap_worst += gap;
                }
