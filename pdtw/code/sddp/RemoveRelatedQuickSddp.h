...
#ifndef REMOVE_RELATED_QUICK_SDDP
#define REMOVE_RELATED_QUICK_SDDP
...
class RemoveRelatedQuickSddp : public RemoveOperator<NodeT,DriverT>
{
	public:
		RemoveRelatedQuickSddp(Relatedness<NodeT,DriverT> * rel):relatedness(rel){}
		~RemoveRelatedQuickSddp(){}
		...
		void Remove(Sol<NodeT, DriverT> & s, int count)
		{
			...
			for(int i = 0; i < s.GetRequestCount(); i++)
			{
				...
				if(s.GetAssignedTo(r->GetParent()) != NULL && !r->GetPickupNode(0)->IsFixed() )
					vect.push_back(r);
			}
