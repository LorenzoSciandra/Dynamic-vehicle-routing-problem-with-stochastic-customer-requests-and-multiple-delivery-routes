...
public:
    ...
    void Show() {
        if (type == NODE_TYPE_PICKUP) {
            printf("Node:%3d OrigId:%3d type:pick x:%4.1lf y:%4.1lf distID:%3d req:%d rel:%3d tw:%3d-%3d drop:%d ", id,
                   origin_id, x, y, distID, req_id, (int) release_time, (int) tw_start, (int) tw_end, del_node_id);
            ...
            printf("Node:%3d OrigId:%3d type:drop x:%4.1lf y:%4.1lf distID:%3d req:%d rel:%3d tw:%3d-%3d pick:%d ", id,
                   origin_id, x, y, distID, req_id, (int) release_time, (int) tw_start, (int) tw_end, pick_node_id);
            printf("Arr:%.1lf Dep:%.1lf Real:%d ", arrival_time, departure_time, (int) is_real);
            printf("Fixed:%d FixedRoute:%d action:%d ", (int) is_fixed_pos, (int) is_fixed_route, action_type);
            if (arrival_time > tw_end) printf("\t <------- infeasible tw");
            printf("\n");
        } else if (type == NODE_TYPE_START_DEPOT)
            printf("Node:%3d type:stad distID:%3d lw:%4d uw:%4d x:%4.1lf y:%4.1lf\n", id, distID, (int) tw_start,
                   (int) tw_end, x, y);
