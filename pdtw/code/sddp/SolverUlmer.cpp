...
SolverUlmer::SolverUlmer(Prob<Node, Driver> & pr) : prob(pr), cost_func(&pr),method(&pr, cost_func),method_ulmer(&pr, cost_func), method_tsp(&pr),sol(&pr,&cost_func)
{
	sol.PutAllNodesToUnassigned();
	alns_iteration_count = Parameters::GetAlnsIterations();
}

void SolverUlmer::Optimize()
{
	//printf("SolverUlmer::Optimize()\n");
	//prob.Show();
	//sol.Show();
	...
	InsRmvMethod<Node,Driver, MoveSddp> * insrmv = &method;
	if(Parameters::GetCurrentTime() <= 0.01)
		insrmv = &method_tsp;
	...
	if(Parameters::GetCurrentTime() <= 0.01) opt2.Optimize(sol);
	//sol.Show();
	...
	for(int i=0;i<sol.GetUnassignedCount();i++)
	{
		...
		if(n->type == NODE_TYPE_PICKUP) {k++; nb_unassigneds++;}
...
void SolverUlmer::GetDecisions(Decisions & decs)
{
	decs.Clear();
	decs.cost = cost;
	...
	for(int i=0;i<sol.GetUnassignedCount();i++)
	{
		...
		if(n->type == NODE_TYPE_PICKUP)
		{
			...
			dec.is_still_feasible = Parameters::GetCurrentTime() + prob.GetTime(0, drop->distID) <= drop->tw_end;
		}
...
void SolverUlmer::SetDecisions(Decisions & decs)
{
	sol.PutAllNodesToUnassigned();
	//decs.Sort();
	...
	for(int i=0;i<decs.GetCount();i++)
	{
		...
		if(dec->decision_type == DECISION_TYPE_ASSIGNMENT)
		{
			...
			r->GetNode(0)->is_assigned_to = dec->driver_id;
			r->GetNode(1)->is_assigned_to = dec->driver_id;
			//printf("Req:%d is assigned to:%d\n", dec->req_id, dec->driver_id);
			continue;
		}
		else if(dec->decision_type == DECISION_TYPE_ACTION)
		{
			Request<Node> * r = sol.GetRequest( dec->req_id );
			if(r == NULL || r->GetNodeCount() != 2 || dec->req_id >= sol.GetRequestCount() || r->id >= sol.GetRequestCount())
			{
			...
			if(r->GetNode(0)->action_type == DECISION_ACTION_NOT_DEFINED || r->GetNode(0)->action_type == DECISION_ACTION_DONT_UNASSIGN)
			{
		...
		n1->is_fixed_pos = dec->prev_departure_time <= Parameters::GetCurrentTime() || !Parameters::AllowEnRouteDepotReturns();
...
void SolverUlmer::MoveLatestPickupFirst()
{
	for(int i=0;i<sol.GetDriverCount();i++)
	{
		Driver * d = sol.GetDriver(i);
...
void SolverUlmer::EvaluateAllWait()
{

	//if(Parameters::GetCurrentTime() >= 479.9)
	{
	//	printf("EvaluateAllWait()\n");
	//	sol.Show(sol.GetDriver(7));
	}

	std::vector<int> actions(sol.GetNodeCount(), DECISION_ACTION_NOT_DEFINED);

	for(int i=0;i<sol.GetDriverCount();i++)
		for(Node * n = sol.GetNode( sol.GetDriver(i)->last_done_node_id ); n->type != NODE_TYPE_END_DEPOT ; n = sol.GetNext(n))
			if(n->IsCustomer())
			{
...
int SolverUlmer::GetFutureCustomerAssignmentCount()
{
	int nb = 0;
	for(int i=0;i<sol.GetDriverCount();i++)
		for(Node * n = sol.GetNode( sol.GetDriver(i)->last_done_node_id ); n->type != NODE_TYPE_END_DEPOT ; n = sol.GetNext(n))
...
int SolverUlmer::GetFutureCustomerCount()
{
	int nb = 0;
	for(int i=0;i<sol.GetCustomerCount();i++)
		if(sol.GetCustomer(i)->type == NODE_TYPE_PICKUP && sol.GetCustomer(i)->is_real && sol.GetCustomer(i)->release_time >= 0.1)
			nb++;
	return nb;
}

int SolverUlmer::GetInitialCustomerUnassignmentCount()
{
	int nb = 0;
	for (int i =0 ; i < sol.GetUnassignedCount() ; i++)
	{
...
void SolverUlmer::GetSolutionCompact(SolCompact<Node,Driver> & s)
{
	s.unassigneds.clear();
	s.routes.clear();
