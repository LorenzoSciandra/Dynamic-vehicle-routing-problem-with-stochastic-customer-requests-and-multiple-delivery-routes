...
public:
	...
	result(int i,double d,int fc, int fa, int ku,double t,int r,int e, int c):id(i),distance(d),future_customers(fc),
																future_assignments(fa),known_unassignments(ku),
																time_taken(t),nb_routes(r),nb_events(e),nb_customers(c){}
	...
	void Show()
	{
		...
		printf("Cust:%d Dist:%.2lf FC:%d KU:%d FA:%d Routes:%d Time:%.2lf Events:%d ", nb_customers, distance, future_customers,known_unassignments,future_assignments , nb_routes, time_taken, nb_events);
		printf("\n");
	}

	void Print(FILE * f)
	{
		fprintf(f,"%.2lf;%d;%d;%.3lf;%d;%d;",distance, known_unassignments, future_assignments, time_taken,nb_routes, nb_events);
	}
};
...
class config
{
	...
	config(int a, int d, int t, int s, int dep):alns(a),driver_count(d),time_horizon(t),scenario_count(s),depot(dep){}
	...
	void PrintAverages(FILE * f)
	{
		...
		for(int i=0;i<=max_id;i++)
			if(nb[i] >= 1)
			{
				...
				fprintf(f,"%.3lf;%.3lf;%.3lf;%.3lf;%.3lf;%.3lf;%.3lf;",dist[i], ku[i], fa[i], routes[i], time[i], events[i],filled[i]);
			}
		fprintf(f,";");
	}
};
...
void Execute(Scenarios & scenarios, config & conf)
{
	...
	printf("P:%d Scenarios:%d Customers:%d Depot:%d\n", Parameters::GetP(), Parameters::GetScenarioCount(), scenarios.GetRealScenarioCustomerCount()/2, conf.depot);
	...
	int fa = sim.future_assignments;
	int ku = sim.known_unassignments;
	conf.results.push_back( result(2,sim.distance,fc,fa,ku,sim.time_taken,sim.nb_routes, sim.nb_events,custs) );
	}
	...
	int fa = sim.future_assignments;
	int ku = sim.known_unassignments;
	conf.results.push_back( result(3,sim.distance,fc,fa,ku,sim.time_taken,sim.nb_routes, sim.nb_events,custs) );
	}
