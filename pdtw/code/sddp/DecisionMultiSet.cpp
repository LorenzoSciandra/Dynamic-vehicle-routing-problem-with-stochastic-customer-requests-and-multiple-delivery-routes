...
void DecisionMultiSet::GetConsensusStacy(Decisions &decisions)
{
	...
	for(size_t i=0;i<reports.size();i++)
	{
		...
		for(int k=0;k<reports[i].GetCount();k++)
		{
			...
			{
				if(routes_sets[i][ d->driver_id ].size() >= 1 && routes_sets[i][ d->driver_id ].back() != -1 && d->node_type == NODE_TYPE_PICKUP)
			...
			if(k != i)
			{
				...
				for(size_t j=0;j<routes_sets[i].size();j++)
				{
					...
					for(size_t l=0;l<routes_sets[k].size();l++)
						if(routes_sets[i][j].size() == routes_sets[k][l].size() && routes_sets[i][j] == routes_sets[k][l])
						{
							nbtimes++;
							break;
						}
...
int DecisionMultiSet::GetConsensusLevel()
{
	...
	for(size_t i=0;i<reports.size();i++)
	{
		...
		for(int k=0;k<reports[i].GetCount();k++)
		{
			...
			if(!driver_at_depot[d->driver_id]) continue;
			if(!d->is_real) driver_stop_adding[ d->driver_id  ] = true;
			if(routes_sets[i][ d->driver_id ].size() >= 1 && d->node_type == NODE_TYPE_PICKUP) driver_stop_adding[ d->driver_id  ] = true;
			if(driver_stop_adding[ d->driver_id  ]) continue;
...
void DecisionMultiSet::GetNextAssignmentDecisions(Decisions & taken_decisions, Decisions & decisions_to_value, Scenarios & sc)
{
	...
	if(best_obj_req != -1)
	{
		for(int j = 0; j < assignments[best_obj_req].size(); j++)
			if(assignments[best_obj_req][j] >= 1 || Parameters::AssignToAllDrivers() || (j == 0 && Parameters::IncludeUnassignmentDecision()) )
			{
				Decision d;
...
void DecisionMultiSet::GetNextAssignmentDecisions2(Decisions & taken_decisions, Decisions & decisions_to_value, Scenarios & sc)
{
...
void DecisionMultiSet::GetNextActionDecisions(Decisions & taken_decisions, Decisions & decisions_to_value, Scenarios & sc)
{
	...
	for(int i=0;i<=max_req_id;i++)
	{
		...
		if(go_now_requests[i] >= 1 && max_v < best_obj && max_v > 0 && Parameters::GetDecisionSelectionMode() == DECISION_SELECTION_MODE_LEAST_COMMON)
		...
		if(go_now_requests[i] >= 1 && max_v > best_obj && max_v < reports.size() && max_v > 0 && Parameters::GetDecisionSelectionMode() == DECISION_SELECTION_MODE_MOST_COMMON)
	...
	if(best_req != -1)
	{
		...
		if(!Parameters::PerformOrderAcceptancy() && (!Parameters::EvaluateRejectOnlyIfNot0() || dont_deliver_requests[best_req] >= 1))
		{
			d.decision_type = DECISION_TYPE_ASSIGNMENT;
			d.driver_id = -1;
			decisions_to_value.Add(d);
...
void DecisionMultiSet::GetMajorityActionDecisions(Decisions & taken_decisions, Decisions & decisions_to_value, Scenarios & sc)
{
