...
void BranchRegretSimulation::Optimize(Scenarios & scenarios)
{
	...
	printf("BranchRegret StartTime:%.1lf P:%d Alns:%d OA:%d Sc:%d BR:%d\n", cur_time, Parameters::GetP(), Parameters::GetAlnsIterations(), (int)Parameters::PerformOrderAcceptancy(), scenarios.GetScenarioCount(), Parameters::GetBranchAndRegretToUse());
	...
	for(nb_events = 1;cur_time <= Parameters::GetTimeHorizon();nb_events++)
	{
		...
		if(!wait_to_next_event)
		{
			...
			if(Parameters::EvaluateWaitingStrategy() && multiset_wait.GetAverageCost() <= 1.001 * multiset.GetAverageCost())
			{
		...
		{
			case BRANCH_AND_REGRET_GONOW_WAIT: multiset.GetNextActionDecisions(working_dd, dd, scenarios); break;
			case BRANCH_AND_REGRET_ASSIGN_TO: multiset.GetNextAssignmentDecisions(working_dd, dd, scenarios); break;
			case BRANCH_AND_REGRET_ASSIGN_TO_AND_DONT: multiset.GetNextAssignmentDecisions2(working_dd, dd, scenarios); break;
		}
		...
		while(dd.GetCount() > 0)
		{
			...
			for(int i=0;i<dd.GetCount();i++)
			{
				...
				if(max_real_un_count <= current_un_count + 0.01 && dd.Get(i)->decision_type == DECISION_TYPE_ASSIGNMENT && dd.Get(i)->driver_id == -1)
					continue;
				...
				for (int j=0;j<Parameters::GetScenarioCount();j++)
				{
					...
					if(!decisions.HasDecisionCharacteristics( dd.Get(i) ) && !decisions.HasDecisionReqDriver(dd.Get(i)->req_id,-1) )
					{
			...
			switch(Parameters::GetBranchAndRegretToUse())
			{
				case BRANCH_AND_REGRET_GONOW_WAIT: best_set.GetNextActionDecisions(working_dd, dd, scenarios); break;
				case BRANCH_AND_REGRET_ASSIGN_TO: best_set.GetNextAssignmentDecisions(working_dd, dd, scenarios); break;
				case BRANCH_AND_REGRET_ASSIGN_TO_AND_DONT: best_set.GetNextAssignmentDecisions2(working_dd, dd, scenarios); break;
			}
			//dd.Show();
		}
		...
		{
			case CONSENSUS_STACY: best_set.GetConsensusStacy(prev_decisions); break;
			case CONSENSUS_BY_DRIVERS: best_set.GetConsensusByDrivers(prev_decisions); break;
			case CONSENSUS_BY_MINIMAL_CHANGE: best_set.GetConsensusMinimalDistance(prev_decisions); break;
		}
	...
	switch(Parameters::GetConsensusToUse())
	{
		case CONSENSUS_STACY: final_solution.name = "BranchAndRegret-RS"; break;
		case CONSENSUS_BY_DRIVERS: final_solution.name = "BranchAndRegret-AS"; break;
		case CONSENSUS_BY_MINIMAL_CHANGE: final_solution.name = "BranchAndRegret-ED"; break;
	}
...
void BranchRegretSimulation::PerformOrderAcceptancy(std::vector< Prob<Node,Driver> >& probs, Decisions& decs, double cur_time, DecisionMultiSet & multiset, Decisions & fixed_decisions)
{
	...
	for(size_t r=0;r<new_requests.size();r++)
	{
		...
		double cost_assign = 0; int sum_real_un = 0;
...
int BranchRegretSimulation::GetUnassignCustomersOA(Decisions & decisions)
{
	...
	for(int i=0;i<decisions.GetCount();i++)
	{
		...
		if(d->node_type == NODE_TYPE_PICKUP && d->decision_type == DECISION_TYPE_ACTION && d->action_type == DECISION_ACTION_DONT_UNASSIGN)
	...
	{
		Decision * d = decisions.Get(i);
		if(d->node_type == NODE_TYPE_PICKUP && d->decision_type == DECISION_TYPE_ASSIGNMENT && d->driver_id == -1 && accepted_orders[d->req_id])
			nb_un_oa++;
	}
	return nb_un_oa;
}
