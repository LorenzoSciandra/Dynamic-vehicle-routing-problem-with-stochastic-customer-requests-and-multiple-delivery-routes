...
void Scenarios::Load(char * filename)
{
	...
	char line1[30];char line2[30];char line3[30];char line4[30];char line5[30];char line6[30];char line7[30];

	int rrr = 0;
	char * rrc = NULL;

	rrr = fscanf(f, "%s %s %s %d", line1, line2, line3, &number_scenarios);
	rrr = fscanf(f, "%s %s %s %d %s %s %s %f", line1, line2, line3, &number_drivers, line4, line5, line6, &driversSpeed);
	rrr = fscanf(f, "%s %s %s %d", line1, line2, line3, &vehicle_capacity);
	rrr = fscanf(f, "%s %s %s %s", line1, line2, line3, line4);
	rrr = fscanf(f, "%s %s %f %s %f %s %s %d %s %d %s\n", line1, line2, &x_dep, line3, &y_dep, line4, line5, &start_time, line6, &end_time, line7);
	Node dep;
	dep.x = x_dep;
	dep.y = y_dep;
	dep.tw_start = 480;
	...
	for(int i = 0; i < number_scenarios; i++) {
		...
		rrr = fscanf(f, "%s %s %s %s %d %s %s %s %d", line1, line2, line3, line4, &type_scenario, line5, line6, line7, &number_clients);
...
void Scenarios::LoadStacyVoccia(std::string filename)
{
	...
	if(myfile.is_open()) {
		while(!myfile.eof()) {
			...
			if(word == "NB_DRIVERS") { int nb_drivers;myfile >> nb_drivers; Parameters::SetDriverCount(nb_drivers); }
			if(word == "INPUT_PATH") { myfile >> path_input;}
			if(word == "OUTPUT_PATH") { myfile >> path_output; }
			if(word == "REAL_REQUEST_WEIGHT") { myfile >> real_weight; Parameters::SetRealRequestWeight(real_weight); }
			if(word == "INSTANCE_TYPE") {
				myfile >> ins_type;/* cout << "Ins " << ins_type << endl;*/ if(ins_type == "Stacy")Parameters::SetStacyInstance(true); if(ins_type == "Own_Instance")Parameters::SetStacyInstance(false);
			}
	...
	cout << "Instace Type:Stacy Problem:" << problem_name_tw << " Actual:" << scenario_id << " TwType:" << tw_type << endl;
	...
	}
	{
		...
		for(int i=1;i<=30;i++)
		{
			...
			if(i < 10) sce += "0";
			sce += std::to_string(i);
			LoadStacyVocciaSubFile((string) "../instances/sddp/stacy/" + type + "/" + problem_name + "_actual/" + problem_name + "_actual_" + sce + ".txt");
		...
		LoadStacyVocciaSubFile((string) "../instances/sddp/stacy/" + type + "/" + problem_name + "_actual/" + problem_name + "_actual_" + scenario_id + ".txt");
       ...
       // cout<<"../instances/Data_Sets/" + type + "/" + problem_name + "_sampled/" + problem_name + "_sampled_" + scenario_id + ".txt"<<endl;
			LoadStacyVocciaSubFile((string) "../instances/sddp/stacy/" + type + "/" + problem_name + "_sampled/" + problem_name + "_sampled_" + scenario_id + ".txt");
		...
		for(int j = 0; j < scenarios[k].GetNodeCount(); j++)
				...
				Parameters::GenerateIntelligentScenario()) )
			{
				...
				if(Parameters::GenerateIntelligentScenario())
				{
					bool has_one = false;
					for(int i=0;i<real.GetNodeCount();i++)
						if(real.GetNode(i)->release_time <= time+0.001 && real.GetNode(i)->type == NODE_TYPE_PICKUP &&
				...
				if (is_stacy)
				{
					...
					if(std::max(n1.release_time + dd, n2.tw_start) + dd > depot_end_time) {inf_request_count++; continue;}
					if(n1.release_time + dd > n2.tw_end) {inf_request_count++; continue;}
				}
		...
		else if (is_ulmer)
		{
			...
			for(int i=0;i<p->GetNodeCount();i++)
			{
				...
				for(int j=0;j<p->GetNodeCount();j++)
				{
					...
					if( (n1->type == NODE_TYPE_START_DEPOT || n1->type == NODE_TYPE_PICKUP || n1->type == NODE_TYPE_END_DEPOT) &&
						 (n2->type == NODE_TYPE_START_DEPOT || n2->type == NODE_TYPE_PICKUP || n2->type == NODE_TYPE_END_DEPOT))
						dist = 0;
...
{
	sc.is_ulmer = true; sc.is_stacy = false;
