...
public:
	...
	result(int i,int c, double d,int u,double t,int r,int e):
		id(i),nb_customers(c),distance(d),unassigneds(u),time_taken(t),nb_routes(r),nb_events(e),unassigneds_oa(-1),nb_branchings(-1){}
	result(int i,int c, double d,int u,double t,int r,int e, int oa):
		id(i),nb_customers(c),distance(d),unassigneds(u),time_taken(t),nb_routes(r),nb_events(e),unassigneds_oa(oa),nb_branchings(-1){}
	...
	void Show()
	{
		...
		printf("Alns:%4d Drivers:%2d TH:%3d SC:%2d Algo:%s ", alns,driver_count,time_horizon, scenario_count,name.c_str());
		printf("Dist:%.2lf Un:%d Routes:%d Time:%.2lf Events:%d ", distance, unassigneds, nb_routes, time_taken, nb_events);
