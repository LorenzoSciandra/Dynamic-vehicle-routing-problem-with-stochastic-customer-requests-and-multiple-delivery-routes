 ...
 *
 *
 * It only tests the dynamic and the consensus function
*/
...
int main(int arg, char ** argv)
{
	...
	//configs.push_back(config(5,10,60,30));
	configs.push_back(config(50,10,60,30));
	configs.push_back(config(100,10,60,30));
	configs.push_back(config(250,10,60,30));
	configs.push_back(config(500,10,60,30));
	configs.push_back(config(1000,10,60,30));
	//configs.push_back(config(2000,10,9999,30));

	for(size_t i=0;i<configs.size();i++)
		Execute(scenarios, configs[i]);

	printf("File:%s\nInstance:%s Customers:%d\n", argv[1], scenarios.problem_name_tw.c_str(), scenarios.GetRealScenarioCustomerCount()/2);
	for(size_t i=0;i<configs.size();i++)
		configs[i].Show();
	...
	if(arg >= 3)
	{
		...
		if(f != NULL)
		{
			fprintf(f,"%s;%s;%d;", argv[1], scenarios.problem_name_tw.c_str(),scenarios.GetRealScenarioCustomerCount()/2);
			for(size_t i=0;i<configs.size();i++)
				configs[i].Print(f);
			fprintf(f,"\n");
			fclose(f);
...
void Execute(Scenarios & scenarios, config & conf)
{
	...
	Parameters::SetEvaluateRejectOnlyIfNot0(false);
...
/*
	...
	Parameters::SetConsensusToUse( CONSENSUS_BY_DRIVERS );
	Parameters::SetDecisionSelectionMode(DECISION_SELECTION_MODE_LEAST_COMMON);
	Parameters::SetEvaluateWaitingStrategy(false);
	sim.Optimize(scenarios);
	conf.results.push_back( result(9,c,sim.distance,sim.unassigneds, sim.time_taken,sim.nb_routes, sim.nb_events) );
	conf.results.back().nb_branchings = sim.nb_branchings;
	}
	...
	Parameters::SetEvaluateRejectOnlyIfNot0(true);
	sim.Optimize(scenarios);
	conf.results.push_back( result(9,c,sim.distance,sim.unassigneds, sim.time_taken,sim.nb_routes, sim.nb_events) );
	conf.results.back().nb_branchings = sim.nb_branchings;
	}
