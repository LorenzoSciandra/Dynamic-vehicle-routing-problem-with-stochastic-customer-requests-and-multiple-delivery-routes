...
InsRmvMethodSddp::InsRmvMethodSddp(Prob<Node, Driver> *prob, CostFunctionSddp &cost_func) : p(prob),
...
void InsRmvMethodSddp::ApplyInsertMove(Sol<Node, Driver> &s, Move<Node, Driver, MoveSddp> &m) {
    ...
    if (s.GetNext(pp)->type == NODE_TYPE_PICKUP)
        while (pp->id != m.to->last_done_node_id && pp->type == NODE_TYPE_PICKUP &&
               pp->release_time > m.n->release_time)
            pp = s.GetPrev(pp);
