...
public:
	...
	double GetAverageCost(){double c = 0; for(size_t i=0;i<reports.size();i++) c += reports[i].cost; return c/reports.size();}
	double GetAverageDist(){double c = 0; for(size_t i=0;i<reports.size();i++) c += reports[i].distances; return c/reports.size();}
	double GetAverageUn(){double c = 0; for(size_t i=0;i<reports.size();i++) c += reports[i].nb_unassigneds; return c/reports.size();}
	double GetAverageRealUn(){double c = 0; for(size_t i=0;i<reports.size();i++) c += reports[i].nb_real_unassigneds; return c/reports.size();}
private:
