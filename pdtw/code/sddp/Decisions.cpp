...
void Decisions::ShowOnlyRealCustomer() {
    for (size_t i = 0; i < decisions.size(); i++)
        if (decisions[i].is_real &&
            (decisions[i].node_type == NODE_TYPE_PICKUP || decisions[i].node_type == NODE_TYPE_DROP))
            decisions[i].Show();
}
...
bool Decisions::HasReqBeenPerformed(int req_id) {
    for (size_t i = 0; i < decisions.size(); i++)
        if (decisions[i].req_id == req_id && decisions[i].driver_id != -1 &&
            decisions[i].arrival_time < Parameters::GetCurrentTime() &&
...
bool Decisions::HasActionDecisionReq(int req_id) {
    for (size_t i = 0; i < decisions.size(); i++)
        if (decisions[i].req_id == req_id &&
            ((decisions[i].decision_type == DECISION_TYPE_ASSIGNMENT && decisions[i].driver_id == -1) ||
...
bool Decisions::HasActionDecisionReqExceptDontUnassign(int req_id) {
    for (size_t i = 0; i < decisions.size(); i++)
        if (decisions[i].req_id == req_id &&
            ((decisions[i].decision_type == DECISION_TYPE_ASSIGNMENT && decisions[i].driver_id == -1) ||
             (decisions[i].decision_type == DECISION_TYPE_ACTION &&
              decisions[i].action_type != DECISION_ACTION_DONT_UNASSIGN)))
            return true;
    return false;
}
    ...
    if (d->decision_type == DECISION_TYPE_ASSIGNMENT) {
        ...
        for (size_t i = 0; i < decisions.size(); i++)
            if (d->req_id == decisions[i].req_id &&
                ...
                if (abs(decisions[i].arrival_time - Parameters::GetCurrentTime()) < 0.001 &&
                    (d->action_type == DECISION_ACTION_GO_NOW || d->action_type == DECISION_ACTION_DONT_UNASSIGN))
                    return true;
                if (decisions[i].arrival_time > Parameters::GetCurrentTime() + 0.001 &&
