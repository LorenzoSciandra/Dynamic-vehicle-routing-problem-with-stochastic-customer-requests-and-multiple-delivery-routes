...
public:
    ...
    void Show() {
        ...
        if (node_id == -1)
            printf("Req:%2d Type:%s Rel:%3.0lf Real:%d ", req_id, Constants_Get_Node_Type(node_type), release_time,
                   (int) is_real);
        else
            printf("NodeID:%3d Req:%2d Type:%s Rel:%3.0lf Real:%d ", node_id, req_id,
                   Constants_Get_Node_Type(node_type), release_time, (int) is_real);

        if (decision_type == DECISION_TYPE_ROUTING) {
            printf("Prev(ID:%3d Type:%s Real:%d ", prev_node_id, Constants_Get_Node_Type(prev_node_type),
                   (int) prev_is_real);
