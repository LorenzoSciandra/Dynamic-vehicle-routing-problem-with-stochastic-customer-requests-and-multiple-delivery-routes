...
public:
	...
	void PerformOrderAcceptancy(std::vector< Prob<Node, Driver> > & probs, Decisions & decs, double cur_time, DecisionMultiSet & multiset, Decisions & fixed_decisions);
	...
	void Show()
	{
		...
		printf("Time:%.2lf Events:%d branching:%d BR_nodes:%d ", time_taken,nb_events, nb_branchings, nb_evaluated_nodes);
	...
	result GetResult()
	{
		...
		{
			case CONSENSUS_STACY: r.name.append("-RS"); break;
			case CONSENSUS_BY_DRIVERS: r.name.append("-AS"); break;
			case CONSENSUS_BY_MINIMAL_CHANGE: r.name.append("-ED"); break;
			default: r.name.append("-Unknown");
		}
		switch(_branching_to_use)
		{
			case BRANCH_AND_REGRET_GONOW_WAIT: r.name.append("-GoNowWait"); break;
			case BRANCH_AND_REGRET_ASSIGN_TO: r.name.append("-AssignTo"); break;
			case BRANCH_AND_REGRET_ASSIGN_TO_AND_DONT: r.name.append("-AssignToAndDont"); break;
			default: r.name.append("-Unknown");
		}
		return r;
	}
