...
void ConsensusSimulation::Optimize(Scenarios & scenarios)
{
	...
	for(nb_events = 1;cur_time <= Parameters::GetTimeHorizon();nb_events++)
	{
		...
		{
			case CONSENSUS_STACY: dec_consensus.GetConsensusStacy(prev_decisions); break;
			case CONSENSUS_BY_DRIVERS: dec_consensus.GetConsensusByDrivers(prev_decisions); break;
			case CONSENSUS_BY_MINIMAL_CHANGE: dec_consensus.GetConsensusMinimalDistance(prev_decisions); break;
		}
	...
	switch(Parameters::GetConsensusToUse())
	{
		case CONSENSUS_STACY: final_solution.name = "Consensus-RS"; break;
		case CONSENSUS_BY_DRIVERS: final_solution.name = "Consensus-AS"; break;
		case CONSENSUS_BY_MINIMAL_CHANGE: final_solution.name = "Consensus-ED"; break;
	}
