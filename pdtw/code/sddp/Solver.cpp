...
void Solver::GetDecisions(Decisions & decs)
{
	...
	for(int i=0;i<sol.GetUnassignedCount();i++)
	{
		...
		if(n->type == NODE_TYPE_PICKUP)
		{
			...
			dec.is_still_feasible = n->serv_time + Parameters::GetCurrentTime()+prob.GetTime(0, drop->distID) <= drop->tw_end &&
				n->serv_time + Parameters::GetCurrentTime()+prob.GetTime(0, drop->distID) + drop->serv_time + prob.GetTime(drop->distID, 0) <= Parameters::GetTimeHorizon();
		}
		else if(n->type == NODE_TYPE_DROP)
		{
			dec.is_still_feasible = Parameters::GetCurrentTime() + prob.GetTime(0, n->distID) <= n->tw_end &&
				Parameters::GetCurrentTime() + prob.GetTime(0, n->distID) + n->serv_time + prob.GetTime(n->distID, 0) <= Parameters::GetTimeHorizon();
			//dec.is_still_feasible = Parameters::GetCurrentTime() + prob.GetTime(0, n->distID) <= n->tw_end;
		}
		decs.Add(dec);
	}
